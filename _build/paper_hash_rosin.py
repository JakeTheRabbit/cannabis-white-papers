...
SUB = ("Warm the resin, push it through a fine mesh bag, and collect what runs out — "
       "that is rosin, made with no solvents. Quality is fixed before the press: "
       "at harvest, in the wash, in the dry. This guide maps the whole chain "
...
SECTIONS.append({"id": "start-here", "kicker": "Start here", "title": "Purpose and scope",
  "blocks": [
    lead("A cannabis plant is covered in thousands of tiny, mushroom-shaped glands called "
         ...
         "all of the plant's potency, smell and flavour. Every solventless concentrate is those heads, "
         "collected and then gently squeezed — with no chemical solvents like butane or alcohol anywhere "
         "in the process."),
    figure(_FIGS["trichome"], 1,
      "Trichomes are the target. Solventless hash-making collects these heads and squeezes "
      "the resin out of them."),
    p("It happens in two halves:"),
    ol(["<strong>Make the hash.</strong> Knock trichome heads off the plant and gather them into a "
        "powder or paste. Two common routes: stir plant material in <em>ice water</em> so the brittle "
        "heads snap off and sink (<strong>bubble hash</strong>), or rub dried material over fine "
    ...
    p("That is the whole idea: <strong>heat plus pressure, no solvents</strong>. The craft is in "
      "restraint — too much heat boils away the flavour, and too much force bursts the bag and pushes "
      "plant matter through the mesh. After pressing, the rosin is <strong>cured</strong> (rested under "
      "controlled warmth or cold) to set its final texture, then either <strong>dabbed</strong> (a small "
      "dose flash-vaporised on a hot surface and inhaled) or loaded into a vape cartridge."),
    ...
    defterm("Micron (µm)", "One-thousandth of a millimetre; the mesh size of the press bag. A smaller "
            "number means finer mesh: cleaner rosin, lower yield."),
    defterm("Cure", "Resting fresh rosin at a set temperature for hours to weeks to fix its texture "
            "and flavour."),
    callout("key", "The one thing this guide builds to",
    ...
    p("From here on, the guide treats the press as a set of <strong>levers</strong> — "
      "temperature, pressure, time and mesh (&lsquo;micron&rsquo;) size — and shows what each lever "
      "does to yield and quality, what goes wrong, and how to fix it. Hit a word you don't know? "
      "Jump to the glossary at the end."),
  ]})

SECTIONS.append({"id": "glossary", "kicker": "The words", "title": "Definitions",
  "blocks": [
    p("Plain-English definitions for the terms this guide uses. Skim once and the technical "
    ...
    table(["Term", "Meaning"], [
      ...
      ["Water activity (a<sub>w</sub>)", "A precise measure of free moisture (0–1) used to judge when material is properly dried (optimal 0.58–0.62)."],
...
SECTIONS.append({"id": "core-answer", "kicker": "The short version", "title": "Core press principles",
  "blocks": [
    p("A rosin press has four levers (<strong>heat, pressure, time and screen (micron)</strong>) "
      "and they all negotiate a single tension: <em>yield versus quality</em> (terpene retention, colour, "
      "clarity)" + _c("pressclub-temp") + ". Heat lowers resin viscosity so it flows — think of cold "
      "honey: stir it and it barely moves, but warm it a few degrees and it pours. The resin in a "
      "trichome behaves the same way; the constraint is that the same warmth that frees the oil also "
      "drives off the lightest aromatic compounds. The micron bag then decides what flows "
      "<em>with</em> the oil (pure trichome oil, or fats and plant matter too); pressure and time only "
      "push the process along" + _c("pressclub-pressure") + ". None of them can lift the ceiling set "
      "upstream."),
    ...
    p("The whole craft reduces to: fix the input, pick the product, choose a micron that matches the "
...
SECTIONS.append({"id": "setpoints", "kicker": "Numbers to start from", "title": "Starting setpoints by input material",
  "blocks": [
    ...
    table(["Input material", "Bag micron", "Plate temp", "Band (°C / °F)", "Dwell (s)", "Pressure approach", "Pre-press", "Typical products"], [
      ...
      ["Dry sift / kief", "72 µm (25–90 by grade)", "82 °C (180 °F)", "71–88 (160–190)", "60–90",
      ...
      ["Dried / cured bubble hash", "45 µm (25–90 by grade)", "77 °C (171 °F)", "71–82 (160–180)", "60–120",
       "Low; a few °C warmer than full-melt to coax flow, but stay gentle", "Cold puck only",
    ...
    callout("note", "Temperature bands at a glance",
      p("<strong>Cold-cure hold (post-press, not platen):</strong> 10–21 °C (50–70 °F) for days to "
        "~2 weeks, sets creamy badder while keeping volatile terpenes" + _c("triminator-coldcure") + ". "
        "<strong>Low-temp press (flavour):</strong> 60–85 °C (140–185 °F), maximum terpene retention, "
        "lightest colour; hash ~60–77 °C (140–171 °F), premium flower ~82–88 °C (180–190 °F). "
        "<strong>Balanced sweet spot (flower):</strong> 88–99 °C (190–210 °F), strong yield, minimal "
        "terpene loss; a good default for unknown <em>flower</em>. "
        "<strong>High-temp press (yield / sauce / carts):</strong> 99–104 °C (210–219 °F), maximum "
        "yield, fastest flow, darker; use it to rescue degraded material" + _c("triminator-tempchart") + ".")),
...
SECTIONS.append({"id": "balances", "kicker": "Core concept", "title": "Four coupled press balances",
  "blocks": [
    ...
    grid([
      card("B-01 · Heat", "Platen heat versus the thermal mass of the bag. Enough heat melts the resin "
           "and drops its viscosity so it flows; too much volatilises terpenes and darkens colour. "
           "<strong>Flow ↔ Flavour.</strong>"),
      card("B-02 · Pressure &amp; flow", "Applied force versus the bag's resistance and the resin's flow "
           "rate. Let force follow the oil, not lead it — like squeezing a full sponge: a slow, steady "
           "squeeze drains cleanly; a sudden burst tears the surface and sprays rather than flows. "
           "Excess force ruptures the bag and drives contaminants through the screen. "
           "<strong>Drive ↔ Blowout.</strong>"),
  ...
  "blocks": [
    ...
    table(["Lever", "What it controls", "Direction of effect"], [
      ...
      ["Pre-press", "Forms a dense, air-free puck",
       "Reduces channelling and blowouts (flower / sift); skip for full-melt hash"],
...
SECTIONS.append({"id": "interaction", "kicker": "Cause and effect", "title": "Press-control interaction matrix",
  "blocks": [
    ...
    p("The pattern to remember: <strong>temperature and micron are your quality levers; pressure and "
  ...
  "blocks": [
    ...
    table(["You observe", "Lever that moved too far", "Corrective change"], [
      ["Low yield, oil won't flow / reabsorbs", "Temp too low · material too dry · pressure too low · screen too tight",
       "+3–6 °C (+5–11 °F); check moisture; firmer, slower ramp; open the micron one step"],
...
SECTIONS.append({"id": "worked-example", "kicker": "One change, traced", "title": "Worked example: tracing one plate temperature change",
  "blocks": [
    p("A single nudge cascades. Take full-melt fresh-frozen hash and raise the platen from "
      "<strong>71 °C → 77 °C (160 °F → 171 °F)</strong>, holding everything else constant."),
    h(3, "Lever pulled"),
    p("Plate temperature +6 °C (+11 °F), still inside the hash band (60–77 °C / 140–171 °F), 36 µm bag, gentle ramp."),
...
SECTIONS.append({"id": "stages", "kicker": "Step by step", "title": "Stage-specific pressing methods",
  "blocks": [
    ...
    ul(["<strong>Fresh-frozen (for live / ice-water hash):</strong> flash-freeze whole, undried material "
        ...
        "<strong>Slow dry (for dry sift):</strong> whole-plant hang at <strong>15–18 °C (59–64 °F), "
        "55–60% RH</strong>, complete darkness, gentle airflow, <strong>10–14 days</strong>. Target "
        "stems that snap (not bend), water activity <strong>0.58–0.62 a<sub>w</sub></strong> "
        "(0.55–0.65 is the safe band; at 0.55 you are already at the overdried boundary). Then freeze "
        "the trimmed material 4–6 h (ideally 24 h) so the stalks snap cleanly" + _c("hightimes-bubbleman") + "."]),
    ...
    p("No water — work frozen material across a descending screen cascade in a cold room (below "
      "10 °C / 50 °F), carding gently 3–5 min per pass" + _c("hightimes-bubbleman") + "."),
    ...
    ul(["<strong>Freeze-dry (lyophilise), best:</strong> condenser −40 to −50 °C (−40 to −58 °F), "
        "vacuum ~100–200 mTorr (below ~500 workable), 24–48 h until fully dry. Removes water without "
        "heat or oxidation.",
    ...
    ul(["Load <strong>2–7 g (0.07–0.25 oz)</strong> into the inner bag; fold the open end over twice. "
        "Don't overfill — leave room for flow.",
        "For diamonds / mech-sep and any blowout-prone run, sleeve the inner bag inside a "
        "<strong>120–160 µm support bag</strong>.",
        "Flower and sift: form a dense, air-free pre-press puck to stop channelling. Full-melt hash: no "
        "heated pre-press — at most a gentle cold-formed puck to consolidate the load; high-grade "
        "material flows without one.",
        "Place in folded parchment, open seam facing out toward you."]),
    h(3, "The press"),
    ol(["Pre-heat the closed (or near-closed) plates against the bag for <strong>30–60 s, no pressure</strong>.",
        "<strong>Slowly ramp</strong> force over 15–20 s, watching for the seam to wet and oil to run.",
        "Hold at working force for <strong>60–120 s</strong> (up to 180 s) or until flow stops — press "
...
SECTIONS.append({"id": "products", "kicker": "Where it ends up", "title": "Post-press product pathways",
  "blocks": [
    ...
    table(["Product", "Press setpoint", "Cure / finish", "Result"], [
      ...
      ["Cold-cure badder / jam", "Cool press", "Sealed jar <strong>10–21 °C (50–70 °F)</strong>, days–2 weeks" + _c("triminator-coldcure"),
       "Creamy, buttery"],
      ["Sauce", "Cool press, then nucleate", "Warm cure", "Wet, high-terpene"],
      ["Diamonds &amp; sauce (mechanical separation)", "Nucleate first (cold cure 13–16 °C / 55–61 °F)",
       "Progressive press <strong>40 → 46 → 52 → 57 °C (104 → 115 → 126 → 135 °F)</strong>" + _c("hashtek-thca-tek") + "; briefly "
       "melt the THCa puck to pour (community practice ~120–130 °C / 248–266 °F, hot enough to start decarb — keep "
      ...
      ["Diamonds (jar / jam tek)", "Press clean rosin",
       "Sealed jar: heat <strong>93 °C (199 °F), 1–2 h</strong>, then crystallise at <strong>38 °C (100 °F) for "
       "1–2 weeks</strong>" + _c("hashtek-jam-tek"), "Crystal balls in jam"],
      ["Vape / cart oil", "Press clean &amp; cool (low-lipid hash is ideal)",
       "Decarb sealed, low and slow: whole rosin at ~71 °C (160 °F) for ~6 days measured only ~3% terpene "
       "loss" + _c("hashtek-decarb") + "; hotter sealed decarbs are faster but cost terps" + _c("lowtemp-carts") + ". "
       "Degas, fridge-test 24 h, fill at ~32 °C (90 °F)" + _c("triminator-carts"), "Clear, stable cart oil"],
    ...
    callout("danger", "The one irreversible rule",
      p("<strong>Never start a diamond press hot.</strong> At high temperature THCa dissolves <em>into</em> "
        ...
        "Think of salt in hot water: the hotter the water, the more stays dissolved and the less "
        "crystallises on cooling. Progressive low-temp pressing removes terpenes gradually so the THCa "
        "runs out of solvent and stays behind. And decarboxylation is one-way" + _c("wang2016-decarb") + ": "
        "decarbed THC will never crystallise" + _c("lowtemp-diamonds") + ".")),
  ...
  "blocks": [
    grid([
      card("F-01 · Blowout", "The bag ruptures and contamination floods the slab. <strong>Cause:</strong> "
           ...
           "matched to head size. Let heat lead; force follows.", tag="rupture"),
      card("F-02 · Low yield / won't flow", "Oil stalls or reabsorbs into the puck. <strong>Cause:</strong> "
           ...
           "<strong>Fix:</strong> +3–6 °C (+5–11 °F); condition moisture; firmer, slower ramp; open the "
           "micron one step.", tag="flow"),
      ...
      card("F-06 · Auto-budders vs won't butter", "Texture with a mind of its own. <strong>Cause:</strong> "
           ...
           "13–16 °C (55–61 °F) for 24–72 h.", tag="texture"),
      card("F-07 · Diamonds won't form", "No crystals, no terpene layer. <strong>Cause:</strong> cure "
           ...
           "the wrong terpene ratio. <strong>Fix:</strong> drop to ~38 °C (100 °F); start from fresh, "
           "undecarbed, nucleated rosin; press progressively — never start hot.", tag="crystals"),
  ...
  "blocks": [
    ...
    table(["Observation", "Likely cause", "First checks"], [
      ["Yield far below 50% on a good grade", "Temp / pressure too low, or material too dry",
       "Confirm platen temp; check moisture; firmer slow ramp; +3–6 °C (+5–11 °F)"],
      ...
      ["Diamonds won't crystallise", "Cure temp too high, or decarbed / un-nucleated rosin",
       "Drop to ~38 °C (100 °F); confirm fresh, nucleated, undecarbed rosin"],
