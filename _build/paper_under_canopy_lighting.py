...
SECTIONS.append({"id": "abstract", "kicker": "Abstract", "title": "Photons at the floor",
  "blocks": [
    lead("A top fixture lights a roof, not a plant. By the time photons fight through three or four leaf "
         ...
         "out, so it doesn&rsquo;t. Under-canopy lighting (SCL) and inter-canopy lighting (ICL) put "
    ...
    p("None of that is free. Every watt you push below the canopy is a watt of heat in the worst-ventilated "
      ...
      "covers the light, the spectrum, the placement, the training, and, the part most guides skip"
      ", the thermal and airflow bill that comes due."),
  ]})
...
SECTIONS.append({"id": "the-evidence", "kicker": "The evidence", "title": "What the research actually shows",
  "blocks": [
    ...
    p("<strong>2025, TL vs SCL vs ICL head to head</strong>, a study in <em>Plants</em> compared "
  ...
  "blocks": [
    ...
    callout("warn", "Photobleaching",
      p("In the Fluence ICL trials, red intercanopy treatments at <strong>80% and 100% red caused "
        "photobleaching</strong>" + _c("fluence-icl-2024") + ", the lower bud bleaches, loses pigment "
    ...
    p("Two things are happening. First, the mid and lower canopy is <em>already</em> green/FR-enriched and "
      "far-red-dominant. The upper leaves absorbed the blue and red on the way down. Stacking more "
    ...
    p("<strong>Far-red (700&ndash;750 nm) is a scalpel, not a default.</strong> Upside: the Emerson effect"
      ", FR added to red/white drives photosynthesis <em>synergistically</em> by balancing PSI and PSII, "
      "and end-of-day FR raised cannabinoid yield in some cultivars" + _c("farred2025-scirep") + ". Downside: "
      "FR is the primary trigger of shade-avoidance, stretch and loose, airy growth, the last thing you "
    ...
    table(["Spectrum", "Verdict", "Why"], [
      ...
      ["+ Far-red (dosed)", "Scalpel", "Photosynthetic synergy + cannabinoid upside, but drives stretch, control it"],
...
SECTIONS.append({"id": "par-targets", "kicker": "Dose", "title": "PAR targets and how much light to add",
  "blocks": [
    ...
    p("Think in <strong>added flux at depth</strong>, not bar wattage. A modest contribution, on the "
      "order of 25&ndash;60 W&middot;m&#8315;&sup2; of installed sub-canopy fixture depending on geometry"
      ", is usually enough to clear the threshold. Past the bleaching point you&rsquo;re paying in heat "
...
SECTIONS.append({"id": "light-history", "kicker": "Photoacclimation", "title": "The leaf was built by yesterday's light",
  "blocks": [
    ...
    p("<strong>One, don&rsquo;t trust a single leaf to predict the canopy.</strong> The same study "
    ...
    p("<strong>Two. The lower leaves you&rsquo;re about to light grew up in shade.</strong> Their "
    ...
    callout("note", "Proven vs plausible",
      p("<strong>Proven:</strong> light history shapes a leaf&rsquo;s photosynthetic capacity, demonstrated in "
        ...
        "cannabis yield. No published trial has tested ramp trajectories against constant intensity. "
...
SECTIONS.append({"id": "placement", "kicker": "Geometry", "title": "Fixture placement and mounting",
  "blocks": [
    ...
    ul(["<strong>Aim up and in.</strong> Simplest to install and clean; lowest disruption to your canopy.",
        ...
        ", bleaching risk scales with proximity &times; red fraction. Broad-spectrum tolerates closer placement.",
    ...
    ul(["<strong>More effective for tall, unpruned genotypes</strong>, exactly the architecture that "
        "benefits most. This is where the strongest study numbers came from.",
        "<strong>Plan plant count down.</strong> Bars in the canopy need lanes, the &lsquo;reduce plant "
...
SECTIONS.append({"id": "training", "kicker": "Training", "title": "Plant training changes when you light from below",
  "blocks": [
    ...
    p("<strong>The gyping reversal.</strong> Standard practice strips the bottom 20 cm, gyping, "
      "lollipopping. Because that growth is shaded, contributes nothing, and invites rot. <strong>Once "
    ...
    ul(["<strong>Keep:</strong> lower bud sites and the leaves directly feeding them, now lit and earning.",
        ...
        "<strong>Tuck before you cut</strong> where you can, redirect shade without removing photosynthetic area.",
...
SECTIONS.append({"id": "heat", "kicker": "Thermal", "title": "The heat bill: a watt is a watt",
  "blocks": [
    p("Ignore the marketing about LEDs &lsquo;running cool&rsquo;. For HVAC sizing it&rsquo;s false in the way "
      ...
      "LED&rsquo;s advantage is hitting your target PPFD at <em>fewer watts</em>. You install fewer "
    ...
    p("That 1,706 BTU/hr sits on top of your overhead array, dehumidifier (nearly 100% of its wattage becomes "
      ...
      "cooling load before SCL. So a retrofit is a direct, calculable increase to your single largest "
    ...
    p("<strong>How to measure your actual load:</strong> sum measured wattage from driver labels and a clamp "
      ...
      "ventilation infiltration; then validate against reality, log the lights-on temp-rise rate; if the "
      "room heats faster than your BTU math predicts, you&rsquo;ve under-counted a load."),
    callout("warn", "The lights-off trap",
      p("Sensible load drops to <strong>zero the instant lights cut</strong>, but the plants keep "
        "transpiring (latent load). If cooling is oversized and dehumidification isn&rsquo;t decoupled, "
        "temperature craters, RH spikes to dew point, and you get condensation on leaves, ideal botrytis "
...
SECTIONS.append({"id": "airflow", "kicker": "Airflow", "title": "The lower-canopy microclimate",
  "blocks": [
    ...
    ul(["<strong>Stagnant dead spots.</strong> Overhead HAF fans sweep the top; the basal zone sits in still "
        ...
        ", exactly where you&rsquo;ve put your new flower."]),
    callout("note", "Sizing the air",
      ul(["Whole-room exchange: target a full air exchange every <strong>1&ndash;3 minutes</strong>",
          ...
          "Carbon-filter penalty: a scrubber adds ~20&ndash;25% static pressure, size the fan +25%, or rate it at 0.25&Prime; static, not free-air"])),
    ...
    ul(["<strong>Dedicated low fans</strong> aimed <em>through</em> the lower canopy, the single "
        "biggest single move once you light the bottom.",
    ...
    p("<strong>VPD and dew point at depth:</strong> manage VPD where the bud is, not just at the room sensor. "
      ...
      "canopy</em>, the basal zone reads wetter than room average, and that delta is exactly the risk "
      "SCL introduces."),
...
SECTIONS.append({"id": "commissioning", "kicker": "Setup", "title": "Commissioning: how to dial it in",
  "blocks": [
    ...
    steps([
      ("Baseline map", "Before fitting bars, take PPFD at apical, middle and basal strata across a grid, quantum sensor, three directions at each point. This is your &lsquo;before&rsquo;."),
      ...
      ("Ramp, don't slam", "Bring sub-canopy intensity up over several days, to catch early bleaching and because shade-developed leaves must re-acclimate before they can use the light (Section 5)."),
      ...
      ("Log per cultivar", "Response is cultivar-dependent, bleaching threshold, stretch under FR, grade uplift. Dense, tall, unpruned genotypes gain most; some tight cultivars gain little and rot easily."),
    ]),
...
SECTIONS.append({"id": "economics", "kicker": "Economics", "title": "The economics, framed honestly",
  "blocks": [
    p("The return on under-canopy lighting is mostly a <strong>grade story</strong>, not a gross-weight story. "
      ...
      "saleable flower, plus reduced variability, worth more in a price-pressured, quality-led market "
      "than raw biomass."),
