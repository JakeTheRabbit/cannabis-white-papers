...
SECTIONS.append({"id": "the-promise", "kicker": "Start here", "title": "The one sentence you’ll leave with",
  "blocks": [
    ...
    callout("tip", "Think of it like driving",
      p("Whether you grow in coco, rockwool, soil or plain water, you steer the exact same way, like "
    ...
    ul(["<strong>The wheel</strong>, how full the sponge is right now (growers call it VWC). "
        "The one number you steer on.",
        "<strong>The engine</strong>, the daily <strong>drink-down</strong> (growers call it dryback): "
        "how far you let the plant wring the sponge dry each day. Your most powerful lever.",
        "<strong>The second dial</strong>, feed strength (growers call it EC): how strong you make the feed."]),
    p("The sponge itself (coco, rockwool, soil, water) is just the <strong>gearbox</strong> "
    ...
    p("Here is the whole routine you&rsquo;re about to learn. Each morning when the lights come on: let the "
      ...
      "out overnight, and start again tomorrow. Everything else is detail hung off this loop."),
    ...
    callout("note", "What&rsquo;s optional here",
      p("Sections 1&ndash;6 and the last one teach you to grow by hand. The two <strong>nerd-bonus</strong> "
        "sections on moisture sensors and auto-watering robots are skippable, if you&rsquo;re growing "
        "by hand, you lose nothing.")),
    callout("key", "In one sentence",
      p("There&rsquo;s one way to steer a plant with water, toward leaves or toward flower, and "
  ...
  "blocks": [
    callout("tip", "Think of it like a kitchen sponge",
      p("Hold a kitchen sponge under the tap until it&rsquo;s full, then let it drip. Every way of growing "
        " (coco, rockwool, soil, even roots dangling in water) is that same sponge: a fixed "
    ...
    fig("fig-4-air-when-wet",
        "Coco keeps about a fifth of its room as air even when soaked; rockwool only a tenth. Which is "
    ...
    p("There&rsquo;s a <strong>second, hidden trait</strong> too, just as important: how much a sponge "
      ...
      _c("noguera2003-cec") + "; others punish you instantly. We&rsquo;ll measure this properly later, "
...
SECTIONS.append({"id": "wheel-and-engine", "kicker": "The wheel and the engine", "title": "Water is the steering wheel, the drink-down is the engine",
  "blocks": [
    ...
    p("<strong>How-full %</strong> is the one number you steer on, how full the sponge is right now. "
    ...
    fig("fig-6-dryback-wring",
        "The drink-down is simply how far the water level drops before you water again, here, 78% down "
    ...
    p("<strong>Why drying steers the plant.</strong> A little daily thirst makes the plant act as if summer "
      ...
      "<strong>nudge, not damage</strong>. You never need the hormone&rsquo;s name to grow."),
    fig("fig-7b-thirst-button",
        "A little thirst makes the plant act as if summer is ending, flipping an inner switch from "
        "grow-leaves to make-flowers, a gentle nudge, not harm."),
    ...
    callout("note", "Real numbers (rockwool example. Yours will differ)",
      ul(["Leaves mode: 5&ndash;15 point drink-down &middot; flower mode: 20&ndash;30 point drink-down",
          ...
          "medicinal yield. Some dry-down is essential, too much hurts."])),
    callout("key", "In one sentence",
      p("You steer the plant by how hard you let it wring the sponge dry each day. Gently for leaves, "
        "hard for flowers.")),
  ]})
  ...
  "blocks": [
    callout("tip", "Think of it like squash on a windowsill",
      p("Make a glass of squash, then leave it on a sunny windowsill. As the water evaporates, the drink gets "
        "stronger and sweeter, same sugar, less water. The root zone does exactly this with the feed.")),
    p("<strong>Feed strength</strong> (growers call it EC) is how strong the fertiliser feed is around the "
      "roots. Higher means stronger and saltier. Here is the one physical fact that ties it to "
    ...
    fig("fig-9-squash-concentration",
        "Count the food dots, always six. As the water drops, the same food crowds closer, so the feed "
    ...
    fig("fig-10b-ec-vwc-together",
        "As the pot dries the feed climbs; every sip resets both, wetness and feed strength move together."),
    callout("note", "Real numbers (rockwool / water. Yours will differ)",
    ...
    callout("note", "Coco growers, one note",
      p("Coco&rsquo;s forgiving sponge soaks up some of the extra feed, so the strength climbs <strong>more "
        ...
        ", coco&rsquo;s is softer.")),
    callout("key", "In one sentence",
      p("When the sponge dries, the water leaves but the food stays, so drying the plant out also makes "
...
SECTIONS.append({"id": "the-cliff", "kicker": "The two limits", "title": "The cliff: the ‘full’ mark and the danger line",
  "blocks": [
    ...
    p("The <strong>&lsquo;full&rsquo; mark</strong> (growers call it field capacity) is the wettest the sponge "
      "gets after it stops dripping, your daily ceiling to refill toward. Crucially, it&rsquo;s a "
    ...
    p("One more friend: the small daily <strong>drip-out</strong> (growers call it runoff), about "
    ...
    fig("fig-13-runoff-dipstick",
        "The few drops that run out carry off built-up salt and let you read the "
    ...
    callout("key", "In one sentence",
      p("There&rsquo;s a &lsquo;full&rsquo; mark you never overflow and a danger line you never fall through"
        ". And steering means swinging the sponge safely between them.")),
  ]})
...
SECTIONS.append({"id": "one-day-four-beats", "kicker": "The daily rhythm", "title": "One day, four beats",
  "blocks": [
    ...
    p("You don&rsquo;t water at random. You water on a fixed <strong>four-beat schedule</strong> tied "
      "to the lights. Growers label the beats P0, P1, P2, P3; you can just call them:"),
    steps([
      ("Dry a little", "At lights-on, let the sponge dry down a small amount before the first sip, the first deliberate squeeze of the day."),
      ("Fill up", "Refill in small, slightly growing sips until you hit the day&rsquo;s &lsquo;full&rsquo; peak. Stop the moment you hit it. You&rsquo;ll rarely give every planned sip."),
    ...
    p("A <strong>sip</strong> (growers call it a shot) is one short timed splash, sized as a small percent of "
      "the sponge&rsquo;s volume. How long to run it, in seconds, is worked out like a recipe, from pot "
    ...
    p("The healthy signature is a <strong>zig-zag</strong> graph (a sawtooth): a gradual fall (drink-down), a "
      ...
      "actually happening, something&rsquo;s broken."),
    ...
    p("Here&rsquo;s the punchline that <em>is</em> the one steering law, visible in one room: "
      ...
      " (drink-down depth, feed strength, sip size) change. Never the rhythm."),
    fig("fig-16-veg-gen-same-loop",
        "Leaves mode and flower mode are the same daily loop. You only slot in a different numbers-card."),
    ...
    callout("key", "In one sentence",
      p("Every steered day is the same four beats, dry a little, fill up, hold steady, wind down "
        "overnight, and that repeating zig-zag is steering working.")),
  ]})
  ...
  "blocks": [
    callout("tip", "Think of it like a gearbox",
      p("You already know how to drive. A gearbox sits between your foot and the wheels, same pedal, "
        ...
        "what the roots feel. Swap it and you don&rsquo;t relearn driving. You just read the new "
        "gearbox&rsquo;s label.")),
    p("This is the keystone. The sponge takes your watering decision and <strong>changes it by fixed "
      "amounts</strong> before the roots feel it, same input, different output per sponge. So learning "
    ...
    fig("fig-17-gearbox-swap",
        "Your watering goes in, the sponge-gearbox changes it, and that is what the roots feel, same "
        "driver and wheel, four swappable gearboxes."),
    p("Every sponge fills in the <strong>same form</strong>, only the numbers differ. That&rsquo;s the "
      "thesis made literal:"),
    ...
    callout("tip", "Start here. A recipe you can run tonight (coco)",
    ...
    callout("key", "In one sentence",
      p("The sponge is a gearbox that only changes four numbers and which mistake will hurt you, the "
  ...
  "blocks": [
    callout("note", "Optional section",
      p("Skip this whole part unless you want to understand the sensors. Growing by hand, you don&rsquo;t need "
        ...
        ", watch the trend, and get a second opinion before you trust it.")),
    ...
    fig("fig-20-permittivity-gap",
        "Water reacts to the probe&rsquo;s electric field far more than dry sponge or air. That gap is "
    ...
    p("<strong>Precise is not the same as right.</strong> The probe shows lots of decimals but can still be "
      "off by a few points" + _c("tdr-fdr-soil-review-2024") + ", like a clock that ticks every second "
    ...
    fig("fig-21-resolution-vs-accuracy",
        "A number with many decimals can still be wrong, like a smooth-ticking clock set ten minutes "
        "fast, until you calibrate it."),
    p("And one probe is one <strong>local witness</strong>. It only feels a soda-can-sized spot, not "
    ...
    fig("fig-23-one-witness-second-witness",
        "One probe lights only a soda-can-sized spot, so confirm with a second witness, drip-out or pot "
        "weight, before any water moves."),
    ...
    callout("key", "In one sentence",
      p("The probe is a fuel gauge, not a dipstick. It can read a bit wrong, so trust the trend and get "
  ...
  "blocks": [
    callout("note", "Optional section",
      p("Skip this unless you&rsquo;re building an automatic waterer. By hand, <em>you</em> are the brain and "
        ...
        "or starve, when unsure, do the safe thing.")),
    ...
    fig("fig-25-water-bank-account",
        "You can know the water level by adding what went in and subtracting what left, a piggy bank"
        ", instead of trusting one twitchy sensor."),
    p("Every guess carries a <strong>confidence level</strong>, high when the clues agree, low when "
    ...
    fig("fig-26-confidence-dial",
        "Only water a full amount when the clues agree; when they don&rsquo;t, do the safe thing, a "
    ...
    p("The whole decision is just three choices, all fenced by one rule. A lying sensor can only ever make the "
      "robot <em>more</em> careful, never trick it into overwatering."),
    fig("fig-27-three-branch-tree",
        "The auto-waterer only ever waters a measured bit, waits, or asks you, all fenced by &lsquo;prefer mild deficit over flooding when uncertain; keep a hard VWC floor&rsquo;."),
    p("And the reassuring part: <strong>the brain works the same no matter the sponge.</strong> The same "
      ...
      "water. The sponge only changes the numbers it learns, never the logic."),
    callout("key", "In one sentence",
      p("An auto-waterer keeps a water piggy-bank, attaches a confidence to its guess, and only ever errs "
        "toward &lsquo;prefer mild deficit over flooding when uncertain; keep a hard VWC floor&rsquo;, on any sponge.")),
  ]})
...
SECTIONS.append({"id": "one-law-any-sponge", "kicker": "It all snaps together", "title": "One law, any sponge",
  "blocks": [
    ...
    p("Everything you learned stacks into one machine: the sponge (gearbox), then how-full % (the wheel), then "
      ...
      "routine. The probe and the auto-brain are optional side-modules, bolt them on only if you automate."),
    p("The same daily zig-zag works in every sponge, only the numbers shift it:"),
    fig("fig-30-one-law-overlay",
        "The same daily shape runs in coco, rockwool, soil and water, only each sponge&rsquo;s own "
        "&lsquo;full&rsquo; mark, danger line and feed strength change."),
    p("Across a whole grow the drink-down has the <strong>same shape on every sponge</strong>, gentle "
      "and wet early, drier and deeper through mid-flower, easing off near the end. Leaves mode is just gentle "
      "cruising; flower mode is firmer driving, on the same road."),
    fig("fig-31-whole-grow-arc",
        "Across the whole grow you steer the same way, gentle early, deepest in mid-flower, easing at "
        "the end, with feed strength climbing alongside."),
    fig("fig-31b-real-plant-strip",
        "What the dials actually do to a living plant, seedling, leafy body, fat flowers, ripening"
        ", start to finish."),
    callout("note", "Real numbers",
      ul(["Forgiveness ladder, most to least: living soil &rarr; coco &rarr; peat &rarr; rockwool &rarr; plain water",
          ...
          ", steer by timing and drink-down, not by starving."])),
    p("There&rsquo;s no magic recipe. Start on the forgiving end (living soil or coco), get a probe on the "
      ...
      "discipline, not a number, is what makes any sponge repeatable. To go deeper, read the "
