...
SECTIONS.append({"id": "intro", "kicker": "Start here", "title": "Purpose and scope",
  "blocks": [
    ...
    p("Because veg feels uneventful (no buds, nothing to weigh) beginners treat it as "
    ...
    p("The working logic: veg exists to build a <strong>frame</strong> (roots, stems, nodes "
      "and leaf) sized to the share of canopy each plant must fill. Plant count, pot size "
      "and target canopy fix the frame; the frame fixes the days. Everything else "
    ...
    callout("note", "Who this is for",
      p("Anyone who has ever flipped 'when it looked big enough', from a first tent to a "
...
SECTIONS.append({"id": "key-terms", "kicker": "Plain-language glossary", "title": "Definitions",
  "blocks": [
    ...
    defterm("Stretch", "The rapid vertical growth in the first 1-3 weeks after the flip, when many "
            ...
            "growth stops."),
    defterm("Canopy", "The top layer of the crop that actually intercepts light, measured "
...
SECTIONS.append({"id": "what-veg-is-for", "kicker": "The why", "title": "Vegetative growth objectives",
  "blocks": [
    ...
    figure(L.bars("Extra flower per plant from longer veg",
      ...
      "Per-plant yield climbs roughly linearly with veg length in the tested range, bigger "
    ...
    p("So veg length is not a lever you push for yield. It is the variable that <em>balances the "
      ...
      "the canopy, no more, no less. Few plants, big shares, long veg. Many plants, small "
    ...
    callout("key", "The core answer",
      p("<strong>Veg for exactly as long as it takes each plant to build its share of the canopy "
        "at or below the maximum flip height, then flip.</strong> Every day short of that "
  ...
  "blocks": [
    ...
    steps([
      ...
      ("Veg until the gate, then flip", "Flip when the canopy is ~80-90% filled, level, and at or "
       "below your maximum flip height (stretch section), a state, not a date. The days "
    ...
    ], cls="compact", caption="Hedged conventions, not lab constants, cultivar vigour, "
    ...
    p("One nuance from the meta-analysis worth knowing: floral <em>biomass</em> favoured short "
      ...
      "cultivar-dependent signal, not a target. Potency is mostly genetics and flowering "
...
SECTIONS.append({"id": "pot-size", "kicker": "The clock inside the pot", "title": "Pot size and vegetative duration",
  "blocks": [
    ...
    figure(L.bars("Double the root volume, roughly +43% plant",
      ...
      "Container volume is a growth ceiling, and small pots hit it fast, mostly through "
    ...
    p("The practical consequence: <strong>every pot size buys a limited veg window</strong> before "
      ...
      "conventions, vigorous cultivars in warm rooms burn through them faster:"),
    table(["Container", "Comfortable veg window", "Suits"], [
      ...
      ["45 L+ / beds", "6+ weeks", "Trees, mother-adjacent frames"],
       ...
       "planned days, or shorten the days to match the pot."),
    callout("danger", "Never flip root-bound",
      p("Stretch roughly doubles water and nutrient demand in three weeks, precisely when "
  ...
  "blocks": [
    ...
    table(["Cultivar type", "Typical multiplier", "Max flip height (H = 100 cm)"], [
      ...
      ["Stretchy / sativa-leaning", "&times;2.5+", "&le;40 cm"],
       ...
       _c("gwe-flowering-stretch") + _c("ilgm-stretch-guide") + ", cultivars vary widely, "
    ...
    callout("tip", "Turn the multiplier into data",
      p("Tag one plant per cultivar. Measure height on flip day and again at day 21 of flower. "
        "Divide. That ratio, written on the strain card, converts next cycle's "
...
SECTIONS.append({"id": "topping", "kicker": "Cutting on schedule", "title": "Topping and FIM: timing and node counts",
  "blocks": [
    p("Topping is the veg tool that turns one dominant tip into two even leaders and a wider, "
      "flatter frame. The full how-and-why lives in "
    ...
    p("The working rule: top when the plant has <strong>4-6 true nodes</strong>, typically 3-4 "
      ...
      "4th and 5th), higher wastes the effect, lower removes too much plant and risks a "
    ...
    figure(_FIGS["topping"], 6,
      ...
      " is paid in veg days, plan it, don't discover it."),
    p("<strong>FIM</strong> (cutting through the top ~&frac34; of the tip rather than below it) "
      ...
      "top."),
    ol([
      "<strong>SOG:</strong> don't top at all. The whole point is one fast cola per plant.",
      ...
      "<strong>Trees:</strong> staged rounds, top, let leaders recover, top the leaders. "
    ...
    callout("warn", "The last-cut deadline",
      p("Count backwards from flip day: <strong>flip minus 10-14 days is the last day for any "
        ...
        "cost, decide it deliberately) or flipping mid-recovery with uneven leaders, which "
...
SECTIONS.append({"id": "canopy-plan", "kicker": "One level, one light", "title": "Canopy establishment in vegetative growth",
  "blocks": [
    ...
    p("Uniformity is also a commercial property, not just an agronomic one: denser, less uniform "
      ...
      "content. Which is grade risk in any market that tests" +
    ...
    ul([
      ...
      "runt never catches up, cull it or bench it separately.",
      ...
      "sideways (low-stress training) so short neighbours close the gap, no recovery time "
      "spent.",
    ...
    callout("note", "A trellis is a plan, not a rescue",
      p("Nets installed in week 3 of flower (over sticky, branched, tangled plants) "
...
SECTIONS.append({"id": "environment", "kicker": "Targets with sources", "title": "Vegetative climate targets",
  "blocks": [
    ...
    figure(L.zones("Veg air temperature: where growth actually runs", 16, 36,
      ...
      ". They're just slower, and slow veg is paid for in days."),
    p("<strong>Light in veg steers shape, not just speed.</strong> In the 21-day trial above, "
      ...
      "produced the compact, sturdy transplants commercial rooms want, with no light-stress "
      ...
      "over days, not in one jump. The ramp itself is covered in "
      "<a href='light-acclimation.html'>light acclimation</a>."),
    figure(L.bars("Veg PPFD by goal",
            [("Fresh transplant", 300), ("Open, airy frame", 600), ("Compact, sturdy", 900)],
            unit="", maxv=1000,
    ...
    callout("warn", "Guard the photoperiod edge",
      p("Modern cultivars are not all '12 hours or nothing': in a ten-cultivar trial, every one "
        ...
        "room drifting toward short days, a failing timer, a mis-set sunrise/sunset "
        "controller, a long power cut, can start flipping plants you meant to keep "
  ...
  "blocks": [
    ...
    table(["Stage", "Feed EC (mS/cm)", "What you're doing"], [
      ...
      ["Last week of veg &rarr; flip", "~2.4-3.0", "Carry EC up so the plant enters stretch with reserves"],
       ...
       "manufacturer stone-wool guidance" + _c("grodan-growguide-steering") + ", exact "
    ...
    p("Two rules of thumb do most of the work. <strong>Colour before calendar:</strong> a veg "
      "plant should hold an even mid-green, pale, yellowing lower leaves mean the N supply "
    ...
    callout("warn", "Pale at flip is a veg mistake paid in flower",
      p("Week-3 lower-leaf yellowing gets blamed on bloom nutrients constantly. The cause is "
        ...
        "entered stretch with no nitrogen reserve. Fix it in veg, by flip day it is "
        "largely pre-paid.")),
  ]})
...
SECTIONS.append({"id": "rootzone", "kicker": "Below the deck", "title": "Root-zone establishment before flowering",
  "blocks": [
    p("The fastest way to lose a week of veg is to drown a fresh transplant. A just-potted plant "
      ...
      "for veg is small, frequent shots, on the order of ~3% of substrate volume, "
    ...
    callout("key", "The pre-flip gate",
      p("All five, or you don't flip. You fix the failing one first. A flip delayed two "
...
SECTIONS.append({"id": "economics", "kicker": "The business of days", "title": "Vegetative duration economics",
  "blocks": [
    ...
    figure(L.bars("Cycles per room per year vs veg length",
      ...
      "full cycle per year. That cost buys bigger plants and fewer of them, whether that's "
      "a good trade depends on what caps you."),
    p("<strong>Where plant count is capped, veg length is the lever you have left.</strong> If a licence "
      "condition, agreement or your own risk model limits how many plants you may run, yield per "
      "plant becomes the metric that matters, and per-plant yield scales with veg length "
    ...
    kv([
      ...
      ("Veg 35+ days", "&asymp;3.7 turns/yr &middot; fewest plants per gram, the count-capped play"),
    ]),
    callout("note", "Decide which currency you're paid in",
      p("Free plant count &rarr; sell time: short veg, high density, maximum turns. Capped plant "
        ...
        "unexamined middle, medium plants, medium density, chosen by habit, optimising "
        "nothing.")),
  ]})
  ...
  "blocks": [
    ...
    grid([
      card("Flipping root-bound",
        p("Pot dries out by lunchtime, roots circling the walls, growth stalled, and it "
      ...
      card("Flipping too tall",
        p("Stretch outruns the ceiling; tops grow into the fixture, bleach and cook, and dimming "
          ...
          "usable height &divide; stretch multiplier, enforced on flip day, no "
      ...
      card("Uneven canopy at flip",
        p("One tall cultivar or a few over-vegged plants pin the light high while the rest "
          "under-run for nine weeks. <strong>Fix:</strong> level in veg, graded clones, "
      ...
      card("Veg with no target",
        p("'One more week' by vibes, no canopy spec, no gate. Every unplanned week is a lost "
          "fraction of a turn, the most expensive habit in the room. <strong>Fix:</strong> "
...
SECTIONS.append({"id": "troubleshooting", "kicker": "Read and fix", "title": "Troubleshooting",
  "blocks": [
    ...
    table(["Symptom", "Likely cause", "What to do"], [
      ...
      ["Pistils/flowers forming in the veg room", "Photoperiod fault, timer, controller or light leak into a &le;14 h day",
      ...
      ["Pot needs water 3&times; a day", "Root-bound, the container clock ran out",
       ...
       "isolation. The same yellow leaf means different things at day 5 and day 25 of veg."),
    callout("key", "The mental model to keep",
      p("<strong>Veg is a countdown to a canopy spec, not a waiting room.</strong> The spec is "
        ...
        "moves the room toward that spec or it's rent. When the spec is met, flip. That "
        "day, not Saturday.")),
