# -*- coding: utf-8 -*-
"""Parse Phase-4 workflow output -> data/refs_gen4.py, data/glossary_gen4.py, _gen/<slug>.json.
Dedupes refs/terms against everything already in the registry."""
import json, os, re

OUT = r"C:\Users\OEM\AppData\Local\Temp\claude\C--Github\88d48e04-5811-43de-a2d7-078d94466ad8\tasks\w720kw5fx.output"
...
for paper in arr:
    ...
    manifest.append((slug, paper["kind"], len(rec["sections"]), len(ref_ids)))

with open(os.path.join(HERE, "data", "refs_gen4.py"), "w", encoding="utf-8") as f:
    ...
    f.write("}\n")
with open(os.path.join(HERE, "data", "glossary_gen4.py"), "w", encoding="utf-8") as f:
    f.write("# -*- coding: utf-8 -*-\n# AUTO-GENERATED Phase-4.\nGLOSSARY_ADD = [\n")
...
print("phase-4 papers:", len(manifest), "| new refs:", len(refs_add), "| new terms:", len(gloss_add))
for m in manifest: print("  %-24s %-12s sections=%-2d refs=%d" % m)
